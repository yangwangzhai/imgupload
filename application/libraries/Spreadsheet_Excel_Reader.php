...
/*
 * Place includes, constant defines and $_GLOBAL settings here. Make sure they
 * have appropriate docblocks to avoid phpDocumentor construing they are
 * documented by the page-level docblock.
 */
...
class Spreadsheet_Excel_Reader
{
     ...
     * --> 'cellsInfo' --> row --> column --> 'type' - Can be 'date', 'number',
     * or 'unknown'
     ...
     * encoding
     * set mb if you would like use 'mb_convert_encoding' for encode UTF-16LE to
     * your encoding
     *
     * @access public
     * @param
    ...
    {
        /*
         * require_once 'OLE.php'; $ole = new OLE(); $ole->read($sFileName);
         * foreach ($ole->_list as $i => $pps) { if (($pps->Name == 'Workbook'
         * || $pps->Name == 'Book') && $pps->Size >= SMALL_BLOCK_THRESHOLD) {
         * $this->data = $ole->getData($i, 0, $ole->getDataLength($i)); } elseif
         * ($pps->Name == 'Root Entry') { $this->data = $ole->getData($i, 0,
         * $ole->getDataLength($i)); } //var_dump(strlen($ole->getData($i, 0,
         * $ole->getDataLength($i))), $pps->Name, md5($this->data),
         * $ole->getDataLength($i)); } //exit; $this->_parse(); return
         * sizeof($this->sheets) > 0;
         */
        ...
        /*
         * $res = $this->_ole->read($sFileName); if ($this->isError($res)) { //
         * var_dump($res); return $this->raiseError($res); } $total =
         * $this->_ole->ppsTotal(); for ($i = 0; $i < $total; $i++) { if
         * ($this->_ole->isFile($i)) { $type = unpack("v",
         * $this->_ole->getData($i, 0, 2)); if ($type[''] == 0x0809) { // check
         * if it's a BIFF stream $this->_index = $i; $this->data =
         * $this->_ole->getData($i, 0, $this->_ole->getDataLength($i)); break; }
         * } } if ($this->_index === null) { return $this->raiseError("$file
         * doesn't seem to be an Excel file"); }
         */
    ...
    function _parse ()
    {
        ...
        $substreamType = ord($this->data[$pos + 6]) |
            ...
            switch ($code) {
                case SPREADSHEET_EXCEL_READER_TYPE_SST:
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i ++) {
                        ...
                        if ($spos == $limitpos) {
                            $opcode = ord($this->data[$spos]) |
                                     ord($this->data[$spos + 1]) << 8;
                            $conlength = ord($this->data[$spos + 2]) |
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = ord($this->data[$spos]) |
                                     (ord($this->data[$spos + 1]) << 8);
                            $spos += 2;
                        }
                        ...
                        if ($extendedString) {
                            // Read in cchExtRst
                            $extendedRunLength = $this->_GetInt4d($this->data, 
                                    $spos);
                            $spos += 4;
                        }
                        ...
                        if ($spos + $len < $limitpos) {
                            ...
                            // found countinue
                            $retstr = substr($this->data, $spos, 
                                    $limitpos - $spos);
                            $bytesRead = $limitpos - $spos;
                            $charsLeft = $numChars -
                                     (($asciiEncoding) ? $bytesRead : ($bytesRead /
                                     2));
                            $spos = $limitpos;
                            ...
                            while ($charsLeft > 0) {
                                ...
                                if ($asciiEncoding && ($option == 0)) {
                                    ...
                                    $len = min($charsLeft * 2, 
                                            $limitpos - $spos); // min($charsLeft,
                                                                                           // $conlength);
                                    $retstr .= substr($this->data, $spos, $len);
                                    $charsLeft -= $len / 2;
                                    $asciiEncoding = false;
                                } elseif (! $asciiEncoding && ($option == 0)) {
                                    // Bummer - the string starts off as
                                    // Unicode, but after the
                                    // continuation it is in straightforward
                                    // ASCII encoding
                                    $len = min($charsLeft, $limitpos - $spos); // min($charsLeft,
                                                                               // $conlength);
                                    for ($j = 0; $j < $len; $j ++) {
                                        $retstr .= $this->data[$spos + $j] .
                                                 chr(0);
                                    }
                                    $charsLeft -= $len;
                                    $asciiEncoding = false;
                        ...
                        $retstr = ($asciiEncoding) ? $retstr : $this->_encodeUTF16(
                                $retstr);
                        // echo "Str $i = $retstr\n";
                        if ($richString) {
                        ...
                        // For extended strings, skip over the extended string
                        // data
                        if ($extendedString) {
                    ...
                    /*
                     * $continueRecords = array(); while ($this->getNextCode()
                     * == Type_CONTINUE) { $continueRecords[] =
                     * &$this->nextRecord(); } //echo " 1 Type_SST\n";
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                    ...
                    if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
                        ...
                        if ($chartype == 0) {
                            ...
                            $rec_name = $this->_encodeUTF16(
                                    substr($this->data, $pos + 12, 
                                            $rec_length * 2));
                        }
    ...
    function _parsesheet ($spos)
    {
        ...
        $substreamType = ord($this->data[$spos + 6]) |
        ...
        while ($cont) {
            ...
            $length = ord($this->data[$spos + 2]) | ord($this->data[$spos + 3]) <<
                     8;
            $spos += 4;
            ...
            switch ($code) {
                case SPREADSHEET_EXCEL_READER_TYPE_DIMENSION:
                    ...
                    if (! isset($this->numRows)) {
                        if (($length == 10) ||
                                 ($version == SPREADSHEET_EXCEL_READER_BIFF7)) {
                            $this->sheets[$this->sn]['numRows'] = ord(
                                    $this->data[$spos + 2]) |
                             ord($this->data[$spos + 3]) << 8;
                    $this->sheets[$this->sn]['numCols'] = ord(
                            $this->data[$spos + 6]) |
                             ord($this->data[$spos + 7]) << 8;
                } else {
                    $this->sheets[$this->sn]['numRows'] = ord(
                    ...
                    $this->sheets[$this->sn]['numCols'] = ord(
                            $this->data[$spos + 10]) |
                             ord($this->data[$spos + 11]) << 8;
                }
            }
            // echo 'numRows '.$this->numRows.' '.$this->numCols."\n";
            break;
        case SPREADSHEET_EXCEL_READER_TYPE_MERGEDCELLS:
            $cellRanges = ord($this->data[$spos]) | ord($this->data[$spos + 1]) <<
                     8;
            for ($i = 0; $i < $cellRanges; $i ++) {
                ...
                // $this->sheets[$this->sn]['mergedCells'][] = array($fr + 1,
                // $fc + 1, $lr + 1, $lc + 1);
                if ($lr - $fr > 0) {
                    $this->sheets[$this->sn]['cellsInfo'][$fr + 1][$fc + 1]['rowspan'] = $lr -
                             $fr + 1;
                }
                if ($lc - $fc > 0) {
                    $this->sheets[$this->sn]['cellsInfo'][$fr + 1][$fc + 1]['colspan'] = $lc -
                             $fc + 1;
                }
            }
        ...
        case SPREADSHEET_EXCEL_READER_TYPE_RK2:
            ...
            $column = ord($this->data[$spos + 2]) | ord($this->data[$spos + 3]) <<
                     8;
        ...
        case SPREADSHEET_EXCEL_READER_TYPE_MULRK:
            ...
            $tmppos = $spos + 4;
            for ($i = 0; $i < $columns; $i ++) {
                $numValue = $this->_GetIEEE754(
                        $this->_GetInt4d($this->data, $tmppos + 2));
                if ($this->isDate($tmppos - 4)) {
                    list ($string, $raw) = $this->createDate($numValue);
                } else {
                    $raw = $numValue;
                    if (isset($this->_columnsFormat[$colFirst + $i + 1])) {
                        $this->curformat = $this->_columnsFormat[$colFirst + $i +
                                 1];
                    }
                    $string = sprintf($this->curformat, 
                            $numValue * $this->multiplier);
                }
                // $rec['rknumbers'][$i]['xfindex'] = ord($rec['data'][$pos]) |
                // ord($rec['data'][$pos+1]) << 8;
                $tmppos += 6;
        ...
        case SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
            ...
            $column = ord($this->data[$spos + 2]) | ord($this->data[$spos + 3]) <<
                     8;
            $tmp = unpack("ddouble", substr($this->data, $spos + 6, 8)); // It
                                                                         // machine
                                                                         // machine
                                                                         // dependent
            if ($this->isDate($spos)) {
     ...
     (ord($this->data[$spos + 12]) == 255) && (ord($this->data[$spos + 13]) ==
     255)) {
    // Error formula. Error code is in +2;
} elseif ((ord($this->data[$spos + 6]) == 3) && (ord($this->data[$spos + 12]) ==
 255) && (ord($this->data[$spos + 13]) == 255)) {
    // Formula result is a null string.
} else {
// result is a number, so first 14 bytes are just like a _NUMBER record
$tmp = unpack("ddouble", substr($this->data, $spos + 6, 8)); // It machine
                                                             // machine
                                                             // dependent
if ($this->isDate($spos)) {
list ($string, $raw) = $this->createDate($tmp['double']);
 ...
 * @return array First element is the converted date, the second element is
...
if ($numValue > 1) {
$utcDays = $numValue -
 ($this->nineteenFour ? SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS);
...
$secs = floor($numValue * SPREADSHEET_EXCEL_READER_MSINADAY) - $hours * 60 * 60 -
 $mins * 60;
...
// for ($i=0; $i<8; $i++) { echo ord($this->data[$i+$spos+6]) . " "; } echo
// "<br>";
...
// echo "Sign = $sign, Exp = $exp, mantissahighx = $mantissa, mantissalow1 =
// $mantissalow1, mantissalow2 = $mantissalow2<br>\n";
if ($sign) {
$value = - 1 * $value;
}
return $value;
...
$this->sheets[$this->sn]['cellsInfo'][$row + $this->_rowoffset][$col +
 $this->_coloffset]['type'] = $this->rectype;
}
...
/*
 * Local variables: tab-width: 4 c-basic-offset: 4 c-hanging-comment-ender-p:
 * nil End:
 */
